                    ...
                    if (blockchainError.isModuleNotFound || errMsg.includes('module_not_found') || errMsg.includes('Module not found')) {
                        setError(
                            <div className="text-left">
                                <p className="font-bold text-red-600 font-sans">Smart Contract Not Found!</p>
                                <p className="text-[11px] mt-1 text-red-500">The module `{MODULE_NAME}` is not deployed to `{MODULE_ADDRESS}` on {NETWORK.toUpperCase()}.</p>
                                <div className="bg-black/10 text-gray-600 p-2 rounded mt-2 text-[10px] font-mono break-all leading-tight border border-black/5 italic">
                                    Tip: Check if your Petra Wallet is on {NETWORK.toUpperCase()}
                                </div>
                                <button
                                    onClick={() => {
                                        setStoreOnChain(false);
